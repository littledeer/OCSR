...
bool similar( const int a, const int b,
              const int percent_dif, const int abs_dif = 1 ) throw();
...
public:
  ...
  Charset() throw() : charset_( 0 ) {}
  bool enable( const char * const name ) throw();
  bool enabled( const Value cset ) const throw();
  bool only( const Value cset ) const throw();
  void show_error( const char * const program_name,
                   const char * const arg ) const throw();
  };
...
public:
  Filter() throw() : type_( none ) {}
  bool set( const char * const name ) throw();
...
public:
  Transformation() throw() : type_( none ) {}
  bool set( const char * const name ) throw();
  Type type() const throw() { return type_; }
  void show_error( const char * const program_name,
                   const char * const arg ) const throw();
...
struct Control
  ...
  bool set_format( const char * const name ) throw();
  };
